...
void track_process(trackTable_strcut *dyn_track_list,
                   gridTrack_t *static_track_list, RadarOutput_Struct *indata,
  ...
  /* 所有航迹状态预测 */
  trace_predict_all(dyn_track_list, static_track_list, vehicleInfo, Tracker_,
                    delta_t);
  RuntimeAnalyze("trace_predict_all");

  /* 点云聚类、匹配 */
  trace_assign_with_box(dyn_track_list, static_track_list, fifo_point,
                        vehicleInfo);
  RuntimeAnalyze("trace_assign_with_box");

  /* 动航迹更新和航迹管理 */
  dyn_trace_update_and_manage(dyn_track_list, fifo_point, vehicleInfo,
                              Tracker_);
 ...
 * @return {*}
 */
void point_cluster_and_fitting(
    std::vector<RadarMeasure_struct> &fifo_point,
  ...
  creat_det_box(clusterSet_dynamic, fifo_point, det_box_list_dynamic,
                DYNAMIC_BOX);
...
void trace_assign_with_box(trackTable_strcut *dyn_track_list,
  ...
  /* 聚类以及box拟合 */
  point_cluster_and_fitting(fifo_point, clusterSet_all, det_box_list_all,
                            vehicleInfo);
  ...
  /* 分配 */
  match_alg(dyn_track_list, static_track_list, trace_info_list_all,
            det_box_list_all, all_assigned_box);

  /* 清空被使用的点云 */
  clear_matched_point(fifo_point, clusterSet_all, det_box_list_all);

  /* 静态航迹更新 */
  static_trace_update_and_merge(static_track_list, det_box_list_all,
                                all_assigned_box);

  /* 运动航迹更新 */
  dynamic_trace_update(fifo_point, dyn_track_list, clusterSet_all,
                       det_box_list_all, all_assigned_box);
}
 ...
 * @return {*}
 */
void static_trace_update_and_merge(
    gridTrack_t *static_track_list, std::vector<det_box_t> &det_box_list_all,
  ...
  /* 统计静态航迹 */
  creat_valid_invalid_trace_list(static_track_list, valid_empty_list,
                                 valid_trace_list);

  /* 航迹更新 */
  static_trace_update(static_track_list, valid_trace_list, det_box_list_all,
                      all_assigned_box);
...
void dynamic_trace_update(
  ...
  dynamic_trace_assigned_with_point(fifo_point, dyn_track_list, TraceSet,
                                    all_assigned_box, clusterSet_static,
                                    det_box_list);
}
...
void creat_trace_box(trackTable_strcut *dyn_track_list,
  ...
  for (const auto sub_trace : valid_trace_list) {
    ...
    new_trace_info.track_type =
        (static_track_list[sub_trace].status == ACTIVATE)
            ? (ACTIVE_STATIC_TRACK)
  ...
  for (const auto sub_trace : dynamic_trace_list) {
    ...
    new_trace_info.track_type =
        (dyn_track_list[sub_trace].trackState == TRACK_STATE_ACTIVE)
            ? (ACTIVE_DYNAMIC_TRACK)
...
void match_alg(trackTable_strcut *dyn_track_list,
  ...
  for (const auto sub_info : trace_info_list) {
    ...
    if ((sub_info.track_type == ACTIVE_DYNAMIC_TRACK) ||
        (sub_info.track_type == UNACTIVE_DYNAMIC_TRACK)) {
      ...
      creat_rect_box_point(trace.kf_info.X_(0), trace.kf_info.X_(1), trace.len,
                           trace.wid, 0.0, trace_box);
    }
  ...
  for (uint32_t trace_idx = 0; trace_idx < trace_box_list.size(); trace_idx++) {
    ...
    if ((sub_info.track_type == ACTIVE_DYNAMIC_TRACK) ||
      ...
      double range_ = sqrt(pow(trace2.kf_info.X_(iDistLat), 2.0) +
                           pow(trace2.kf_info.X_(iDistLong), 2.0));
      double temp_data =
          trace2.kf_info.X_(iDistLat) * trace2.kf_info.X_(iVrelLat) +
          trace2.kf_info.X_(iDistLong) * trace2.kf_info.X_(iVrelLong);
    ...
    for (uint32_t det_box_idx = 0; det_box_idx < det_rect_box_list.size();
         det_box_idx++) {
      ...
      new_pair.box_overlap = intersection_area(
          trace_box_list.at(trace_idx), det_rect_box_list.at(det_box_idx));
      new_pair.iou =
          new_pair.box_overlap /
          (new_pair.trace_box_s + new_pair.det_box_s - new_pair.box_overlap);
  ...
  memset(match_table, false,
         (trace_box_list.size() * det_rect_box_list.size()) * sizeof(bool));
  ...
  for (const auto sub_pair : cost_matrix) {
    ...
    // 确定该航迹是否是确认的运动航迹
    if (sub_det_box.valid & (1 << VALID) ||
        (trace_info_list.at(trace_idx).track_type == ACTIVE_DYNAMIC_TRACK)) {
      if (sub_pair.iou > 0.0)  // 存在交叉
      {
        // 运动航迹的分配逻辑（包含确认航迹与未确认航迹）
        if ((trace_info_list.at(trace_idx).track_type ==
             ACTIVE_DYNAMIC_TRACK) ||
            (trace_info_list.at(trace_idx).track_type ==
             UNACTIVE_DYNAMIC_TRACK)) {
          trackTable_strcut &trace =
              dyn_track_list[trace_info_list.at(trace_idx).trace_idx];

          // 确认运动航迹与动态box的关联逻辑
          if ((trace_info_list.at(trace_idx).track_type ==
               ACTIVE_DYNAMIC_TRACK) &&
              (sub_det_box.box_feature == DYNAMIC_BOX)) {
            if (sub_det_box.assigned_obj == ACTIVE_DYNAMIC_TRACK) {
              // TBD
            } else {
              // 20230616: 增加速度判定 RADAR-Q74
              if (fabs(sub_det_box.v_mean - trace.KalmanInfo.MeasPre(2)) <
                  4.0) {
                //新增余弦残差
                double a1 = atan2(sub_det_box.x_pos, sub_det_box.y_pos);
                double a2 = atan2(trace.KalmanInfo.MeasPre(0),
                                  trace.KalmanInfo.MeasPre(1));
                double diff_ = a1 - a2;
          ...
          if ((trace_info_list.at(trace_idx).track_type ==
            ...
            if (sub_det_box.assigned_obj == ACTIVE_DYNAMIC_TRACK) {
              ...
              double likelihood_pos = ComputeScaleOfEllipse(
                  &trace, sub_det_box.x_pos, sub_det_box.y_pos);
        ...
        if ((trace_info_list.at(trace_idx).track_type == ACTIVE_STATIC_TRACK) ||
            (trace_info_list.at(trace_idx).track_type ==
             UNACTIVE_STATIC_TRACK)) {
          gridTrack_t &static_trace =
              static_track_list[trace_info_list.at(trace_idx).trace_idx];
          ...
          if (sub_det_box.box_feature == STATIC_BOX) {
            if (sub_det_box.member_num >= 4) {
              ...
              if ((box_ratio < 3.0) || (sub_pair.center_dis < 2.0) ||
                  (sub_pair.box_overlap > 1.0)) {
                assinged_flag = true;
              } else {
                // 正前方情况下，存在IOU就关联
                double distance_2_zero = 0.0;
                if (fabs(static_trace.kf_info.X_(iDistLat)) >
                    (0.5 * static_trace.wid)) {
                  distance_2_zero = (static_trace.kf_info.X_(iDistLat) > 0.0)
                                        ? (static_trace.kf_info.X_(iDistLat) -
                                           0.5 * static_trace.wid)
                                        : (-static_trace.kf_info.X_(iDistLat) +
                                           0.5 * static_trace.wid);
                } else {
            ...
            if (box_ratio < 3.0) {
              if (((sub_det_box.assigned_obj == UNACTIVE_DYNAMIC_TRACK) &&
                ...
                // 计算速度差
                double trace_v =
                    (static_trace.kf_info.X_(0) * static_trace.kf_info.X_(2) +
                     static_trace.kf_info.X_(1) * static_trace.kf_info.X_(3)) /
                    (sqrt(pow(static_trace.kf_info.X_(0), 2.0) +
                          pow(static_trace.kf_info.X_(1), 2.0)));
                ...
                if (diff_v < 2.0) {
                  if ((sub_det_box.assigned_obj == UNACTIVE_DYNAMIC_TRACK) ||
                      ((sub_det_box.valid & (1 << VALID)))) {
                    if ((sub_det_box.assigned_obj == UNACTIVE_DYNAMIC_TRACK)) {
                      all_assigned_box.at(sub_det_box.matched_trace_idx)
                          .clear();
                    }
                  }
  ...
  for (uint16_t meas_idx = 0; meas_idx < det_rect_box_list.size(); meas_idx++) {
    ...
    for (uint16_t trace_idx = 0; trace_idx < trace_box_list.size();
         trace_idx++) {
  ...
  for (uint16_t trace_idx = 0; trace_idx < trace_box_list.size(); trace_idx++) {
    for (uint16_t meas_idx = 0; meas_idx < det_rect_box_list.size();
         meas_idx++) {
      if (match_table[trace_idx][meas_idx] == true) {
  ...
  for (const auto sub_pair : cost_matrix) {
    ...
    // 只对确认航迹进行补充关联
    if ((trace_info_list.at(trace_idx).track_type > UNACTIVE_DYNAMIC_TRACK) ||
        (sub_det_box.matched_trace_idx == trace_idx)) {
      continue;
    }
    ...
    if (((sub_det_box.valid & (1 << VALID)) == (1 << VALID)) ||
      ...
      double likelihood_pos =
          ComputeScaleOfEllipse(&trace, sub_det_box.x_pos, sub_det_box.y_pos);
      ...
      if (trace.ExtendInfo.Object_Class < VEHICLE) {
        likeliHood_doppler =
            exp(-0.5 * (pow(diff_v, 2.0) / (R_(2, 2) + pow(0.5 * 2, 2.0))));
      } else {
        likeliHood_doppler = exp(-0.5 * (pow(diff_v, 2.0) / R_(2, 2)));
      }

      bool same_dir =
          ((sub_det_box.v_mean * trace.KalmanInfo.MeasPre(2)) >= 0.0) ? true
                                                                      : false;
      ...
      } else  // static box
      {
        ...
        double likeliHood_doppler =
            exp(-0.5 * (pow(diff_v, 2.0) /
                        (trace.KalmanInfo.R_(2, 2) + pow(0.25 * 2, 2.0))));
        double det_box_size = sub_det_box.len * sub_det_box.wid;

        double box_ratio = (det_box_size / trace_box_size);

        double center_dis = sqrt(
            pow(sub_det_box.x_pos - trace.KalmanInfo.StateEst(iDistLat), 2.0) +
        ...
        if (box_ratio < 5.0) {
          if (trace.ExtendInfo.Object_Class < VEHICLE)  // 小目标不强行要求iou
          {
            ...
            if (((iou_val > 0.1) && (likeliHood_doppler > 0.2)) ||
                ((iou_val > 0.0) && (likelihood_pos > 0.4) &&
                 (likeliHood_doppler > 0.4)) ||
  ...
  for (const auto sub_pair : cost_matrix) {
    ...
    if (((sub_det_box.valid & (1 << VALID)) == (1 << VALID)) ||
      ...
      double det_box_size = sub_det_box.len * sub_det_box.wid;
      double box_ratio = (trace_box_size > det_box_size)
                             ? (trace_box_size / det_box_size)
                             : (det_box_size / trace_box_size);

      double center_dis = sqrt(
          pow(sub_det_box.x_pos - static_trace.kf_info.X_(iDistLat), 2.0) +
          ...
          if ((center_dis < 2.0) && (box_ratio < 4.0)) {
            ...
            if (box_ratio < 1.5) {
              ...
              if (static_trace.kf_info.X_(iDistLat) > sub_det_box.x_pos) {
                closet_side_range = (static_trace.kf_info.X_(iDistLat) -
                                     0.5 * static_trace.wid) -
              ...
              if ((fabs(closet_side_range) < 1.0) &&
                  (fabs(sub_det_box.y_pos -
                        static_trace.kf_info.X_(iDistLong)) < 1.0)) {
                assinged_flag = true;
              }
            }
          }
        }
      } else if ((sub_det_box.box_feature == DYNAMIC_BOX) &&
                 (sub_det_box.member_num > 1)) {
 ...
 * @return {*}
 */
void All_Trace_Mage_Proc(trackTable_strcut *dyn_track_list,
                         gridTrack_t *static_track_list) {
...
bool is_multi_bounce(trackTable_strcut *trace1, trackTable_strcut *trace2) {
  ...
  double trace_range2 =
      sqrt(pow(trace2->KalmanInfo.StateEst(iDistLat) - 2.57837, 2.0) +
  ...
  creat_rect_box_point(trace1->KalmanInfo.StateEst(iDistLat) - 2.57837,
                       trace1->KalmanInfo.StateEst(iDistLong),
                       trace1->ExtendInfo.Length, trace1->ExtendInfo.Width,
                       (-1.0) * trace1->ExtendInfo.box_theta, trace1_box);
  ...
  creat_rect_box_point(jumped_dis_lat, jumped_dis_long,
                       trace2->ExtendInfo.Length * 1.2,
                       trace2->ExtendInfo.Width * 1.5,
                       (-1.0) * trace2->ExtendInfo.box_theta, trace2_box);

  double trace1_doppler_v =
      (trace1->KalmanInfo.StateEst(iDistLat) *
           trace1->KalmanInfo.StateEst(iVrelLat) +
       trace1->KalmanInfo.StateEst(iDistLong) *
           trace1->KalmanInfo.StateEst(iVrelLong)) /
      (sqrt(pow(trace1->KalmanInfo.StateEst(iDistLat), 2.0) +
            pow(trace1->KalmanInfo.StateEst(iDistLong), 2.0)));

  double trace2_doppler_v =
      (trace2->KalmanInfo.StateEst(iDistLat) *
           trace2->KalmanInfo.StateEst(iVrelLat) +
       trace2->KalmanInfo.StateEst(iDistLong) *
           trace2->KalmanInfo.StateEst(iVrelLong)) /
      (sqrt(pow(trace2->KalmanInfo.StateEst(iDistLat), 2.0) +
            pow(trace2->KalmanInfo.StateEst(iDistLong), 2.0)));
  ...
  if (iou_ > 0.0) {
    ...
    if ((trace1->trackState <= trace2->trackState) &&
        (fabs(trace1_doppler_v - trace2_doppler_v * 2.0) < 1.0)) {
      fakeTrace = true;
    }
  } else {
    ...
    if (iou_) {
      if ((trace1->trackState <= trace2->trackState) &&
          (fabs(trace1_doppler_v - trace2_doppler_v * 3.0) < 1.0)) {
        fakeTrace = true;
      }
    }
  }
 ...
 * @return {*}
 */
void compare_between_dynamic_trace(trackTable_strcut *dynamic_trace_1,
                                   trackTable_strcut *dynamic_trace_2) {
  ...
  creat_rect_box_point(dynamic_trace_2->KalmanInfo.StateEst(0),
                       dynamic_trace_2->KalmanInfo.StateEst(1),
                       dynamic_trace_2->ExtendInfo.Length,
                       dynamic_trace_2->ExtendInfo.Width,
                       -1.0 * dynamic_trace_2->ExtendInfo.box_theta, r1);

  creat_rect_box_point(dynamic_trace_1->KalmanInfo.StateEst(0),
                       dynamic_trace_1->KalmanInfo.StateEst(1),
                       dynamic_trace_1->ExtendInfo.Length,
                       dynamic_trace_1->ExtendInfo.Width,
 ...
 * @return {*}
 */
void view_grid_test(trackTable_strcut *dyn_track_list,
                    gridTrack_t *static_track_list) {
  ...
  for (uint16_t idx = 0; idx < MAXTRACKS; idx++) {
    if (dyn_track_list[idx].trackState > TRACK_STATE_FREE) {
      ...
      compute_trace_anlge(
          sub_trace->KalmanInfo.StateEst(iDistLat) - RADAR_MOUNT_Y,
          sub_trace->KalmanInfo.StateEst(iDistLong) - RADAR_MOUNT_X,
          0.5 * sub_trace->ExtendInfo.Length, 0.5 * sub_trace->ExtendInfo.Width,
          sub_trace->ExtendInfo.box_theta, &ang_min, &ang_max);
  ...
  for (uint16_t idx = 0; idx < STATIC_TRACK_MAXNUM; idx++) {
    if (static_track_list[idx].status > EMPTY) {
      ...
      compute_trace_anlge(sub_trace->kf_info.X_(iDistLat) - RADAR_MOUNT_Y,
                          sub_trace->kf_info.X_(iDistLong) - RADAR_MOUNT_X,
  ...
  for (auto slaver_trace_ang : trace_angle_list) {
    if (is_dynamic_trace(slaver_trace_ang.first)) {
      ...
      slaver_trance_range =
          sqrt(pow(slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat), 2.0) +
               pow(slaver_dynamic_trace->KalmanInfo.StateEst(iDistLong), 2.0));
    } else {
      slaver_static_trace =
          &static_track_list[slaver_trace_ang.first - MAXTRACKS];
      slaver_dynamic_trace = nullptr;

      slaver_trance_range =
          sqrt(pow(slaver_static_trace->kf_info.X_(iDistLat), 2.0) +
    ...
    for (auto master_trace_ang : trace_angle_list) {
      ...
      if (is_dynamic_trace(master_trace_ang.first)) {
        ...
        master_trace_range = sqrt(
            pow(master_dynamic_trace->KalmanInfo.StateEst(iDistLat), 2.0) +
            pow(master_dynamic_trace->KalmanInfo.StateEst(iDistLong), 2.0));
      } else {
        master_static_trace =
            &static_track_list[master_trace_ang.first - MAXTRACKS];
        master_dynamic_trace = nullptr;

        master_trace_range =
            sqrt(pow(master_static_trace->kf_info.X_(iDistLat), 2.0) +
                 pow(master_static_trace->kf_info.X_(iDistLong), 2.0));
      }

      /* 开始比较 */
      /* 静态目标之间 */
      if ((slaver_static_trace != nullptr) &&
          (master_static_trace != nullptr)) {
        pure_trace_flag = false;
        if (trace_result.at(slaver_idx)) {
          trace_result.at(slaver_idx) =
              compare_static_trace(slaver_static_trace, master_static_trace);
        }
      }

      angle_enum angele_type = compute_two_angle_position(
          slaver_trace_ang.second, master_trace_ang.second);

      if ((angele_type > ANGLE_NULLSET) &&
          (slaver_trance_range > master_trace_range)) {
        pure_trace_flag = false;
      }
      ...
      if ((slaver_dynamic_trace != nullptr) &&
        ...
        if (slaver_trance_range > master_trace_range) {
          ...
          if (angele_type == ANGLE_OVERLAP) {
            if ((master_dynamic_trace->trackState == TRACK_STATE_ACTIVE) &&
              ...
              if (slaver_dynamic_trace->trackState == TRACK_STATE_DETECTION) {
                ...
                // 当前方车处于±2米内，则删除其前方目标
                if (((fabs(
                          master_dynamic_trace->KalmanInfo.StateEst(iDistLat)) -
                      0.5 * master_dynamic_trace->ExtendInfo.Width) < 2.0) &&
                    ((fabs(
                          slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat)) -
                      0.5 * slaver_dynamic_trace->ExtendInfo.Width) < 2.0)) {
                  trace_result.at(slaver_idx) = false;
                }
              }
            }
          ...
          if (angele_type == ANGLE_CROSS) {
            // 补充正前方特殊情况
            if ((fabs(slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat)) <
                 2.0) &&
                (fabs(master_dynamic_trace->KalmanInfo.StateEst(iDistLat)) <
                 2.0)) {
          ...
          if (trace_result.at(slaver_idx)) {
            ...
            trace_result.at(slaver_idx) = (!(
                is_multi_bounce(slaver_dynamic_trace, master_dynamic_trace)));
          }

          // 20230606 add:
          if (trace_result.at(slaver_idx)) {
            // 正前方有辆车(确认航迹)
            if ((fabs(master_dynamic_trace->KalmanInfo.StateEst(iDistLat)) <
                 2.0) &&
                (master_dynamic_trace->trackState == TRACK_STATE_ACTIVE) &&
                (master_dynamic_trace->ExtendInfo.Object_Class >= VEHICLE)) {
              double diff_vlong =
                  fabs(master_dynamic_trace->KalmanInfo.StateEst(iVrelLong) -
                       slaver_dynamic_trace->KalmanInfo.StateEst(iVrelLong));

              // 纵向速度差较小且运动方向一致
              if ((diff_vlong < 1.0) &&
                  ((master_dynamic_trace->KalmanInfo.StateEst(iVrelLong) *
                    slaver_dynamic_trace->KalmanInfo.StateEst(iVrelLong)) >
                   0.0)) {
                double diff_lat =
                    fabs(master_dynamic_trace->KalmanInfo.StateEst(iDistLat) -
                ...
                if (diff_lat <
                    (0.5 * (master_dynamic_trace->ExtendInfo.Width +
                            slaver_dynamic_trace->ExtendInfo.Width))) {
                  diff_lat = 0.0;
                } else {
                  diff_lat =
                      diff_lat - 0.5 * (master_dynamic_trace->ExtendInfo.Width +
                                        slaver_dynamic_trace->ExtendInfo.Width);
                }

                if ((slaver_dynamic_trace->KalmanInfo.StateEst(iDistLong) >
                     master_dynamic_trace->KalmanInfo.StateEst(iDistLong)) &&
                    (diff_lat < 1.0)) {
                  trace_result.at(slaver_idx) = false;
                }
              }
      ...
      if ((slaver_static_trace != nullptr) &&
        ...
        if (fabs(master_dynamic_trace->KalmanInfo.StateEst(iDistLat)) <
            (master_dynamic_trace->ExtendInfo.Width * 0.5)) {
          dis_to_zero_1 = 0.0;
        } else {
          dis_to_zero_1 =
              (master_dynamic_trace->KalmanInfo.StateEst(iDistLat) > 0.0)
                  ? (master_dynamic_trace->KalmanInfo.StateEst(iDistLat) -
                     (master_dynamic_trace->ExtendInfo.Width * 0.5))
                  : (-1.0 *
                         master_dynamic_trace->KalmanInfo.StateEst(iDistLat) -
        ...
        if (fabs(slaver_static_trace->kf_info.X_(iDistLat)) <
            (slaver_static_trace->wid * 0.5)) {
          dis_to_zero_2 = 0.0;
        } else {
          dis_to_zero_2 =
              (slaver_static_trace->kf_info.X_(iDistLat) > 0.0)
                  ? (slaver_static_trace->kf_info.X_(iDistLat) -
                     (slaver_static_trace->wid * 0.5))
                  : (-1.0 * slaver_static_trace->kf_info.X_(iDistLat) -
                     (slaver_static_trace->wid * 0.5));
        }
        ...
        if (angele_type > ANGLE_NULLSET) {
          if ((dis_to_zero_1 < 1.0) && (dis_to_zero_2 < 1.0) &&
              ...
              (master_dynamic_trace->ExtendInfo.Object_Class >= VEHICLE) &&
              (master_dynamic_trace->KalmanInfo.StateEst(iDistLong) <
               slaver_static_trace->kf_info.X_(iDistLong))) {
            trace_result.at(slaver_idx) = false;
          } else {
            if (trace_result.at(slaver_idx)) {
              trace_result.at(slaver_idx) = compare_static_trace_with_dyntrace(
                  slaver_static_trace, master_dynamic_trace);
            }
          }
        } else {
          // 视角完全重叠
      ...
      if ((slaver_dynamic_trace != nullptr) &&
        ...
        if (slaver_trance_range > master_trace_range) {
          // 静态航迹是否在正前方
          if ((((master_static_trace->kf_info.X_(iDistLat) > 0.0) &&
                (master_static_trace->kf_info.X_(iDistLat) -
                 0.5 * master_static_trace->wid) < 1.0) ||
               ((master_static_trace->kf_info.X_(iDistLat) < 0.0) &&
                (master_static_trace->kf_info.X_(iDistLat) +
                 0.5 * master_static_trace->wid) > -1.0)) &&
              (master_static_trace->status == 2)) {
            if (((slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat) > 0.0) &&
                 (slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat) -
                  0.5 * slaver_dynamic_trace->ExtendInfo.Width) < 1.0) ||
                ((slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat) < 0.0) &&
                 (slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat) +
                  0.5 * slaver_dynamic_trace->ExtendInfo.Width) > -1.0)) {
              if ((master_static_trace->wid > 1.0) &&
                  (master_static_trace->len > 1.0)) {
                trace_result.at(slaver_idx) = false;
              }
            }
          }
          ...
          //                    {
          //                        double size_scale =
          //                        (master_static_trace->wid *
          //                        master_static_trace->len ) / \
//                                            (slaver_dynamic_trace->ExtendInfo.Width
          //                                            * slaver_dynamic_trace->ExtendInfo.Length) ;

          //                        if(size_scale > 0.8)
          //                        {
          //                            if((master_static_trace->status == 2) &&
          //                            \ (slaver_dynamic_trace->trackState <
          //                            TRACK_STATE_ACTIVE))
          //                            {
          //                                trace_result.at(slaver_idx) = false;
          //                            }
          //                            else
          //                            {
          //                                if((master_static_trace->status ==
          //                                2) &&
          //                                (fabs(slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat))<
          //                                1.0))
          //                                {
          //                                    trace_result.at(slaver_idx) =
          //                                    false;
...
//                            (slaver_dynamic_trace->trackState <
          ...
          //                        {
          //                            //TODO:
          //                            若运动目标处于本车道，且静态目标也处于本车道，并且该运动目标为靠近状态
          //                            if((fabs(slaver_dynamic_trace->KalmanInfo.StateEst(iDistLat))<
          //                            3.0) && \
//                                (fabs(master_static_trace->kf_info.X_(iDistLat))
          //                                < 3.0) )
          //                            {
          //                                if(((master_static_trace->len*master_static_trace->wid)
          //                                > 4.0) && \
//                                    (slaver_dynamic_trace->KalmanInfo.StateEst(iVrelLong)
          //                                    < 0.0))
          //                                {
          //                                    double angle_iou =
          //                                    compute_two_angle_iou(slaver_trace_ang.second,
          //                                    master_trace_ang.second);
    ...
    if (slaver_dynamic_trace != nullptr) {
      if (pure_trace_flag) {
        ...
        slaver_dynamic_trace->ManageInfo.trace_belief =
            Valuelimit(0.0, 1.0, slaver_dynamic_trace->ManageInfo.trace_belief);
      }
    }
  ...
  for (const auto sub_result : trace_result) {
    if (!sub_result) {
      if (is_dynamic_trace(trace_angle_list.at(slaver_idx).first)) {
        ...
        if (static_track_list[trace_angle_list.at(slaver_idx).first - MAXTRACKS]
                .age < 30) {
          static_track_list[trace_angle_list.at(slaver_idx).first - MAXTRACKS]
              .status = EMPTY;
        }

#ifdef LOG_OUTPUT
        std::cout << " (Line 1470) Delete Trace ID: "
                  << static_track_list[trace_angle_list.at(slaver_idx).first -
                                       MAXTRACKS]
                         .ID
                  << std::endl;
#endif
 ...
 * @return {*}
 */
void compute_trace_anlge(double x_pos, double y_pos, double half_len,
                         double half_wid, double theta, double *angle_min,
                         double *angle_max) {
  std::vector<double> angle_list;
  double plusOrminus[4][2] = {
 ...
 * @return {*}
 */
bool compare_static_trace(gridTrack_t *slaver_trace,
                          gridTrack_t *master_trace) {
  box_t box1, box2;
  ...
  if (iou > 0.3) {
    ...
    if ((slaver_trace->status <= master_trace->status) &&
        (box_size1 <= box_size2)) {
      return false;
    }
  } else {
    ...
    if (fabs(slaver_trace->kf_info.X_(iDistLat)) < (slaver_trace->wid * 0.5)) {
      ...
      distance1 =
          (slaver_trace->kf_info.X_(iDistLat) > 0.0)
              ? (slaver_trace->kf_info.X_(iDistLat) - (slaver_trace->wid * 0.5))
              : (-1.0 * slaver_trace->kf_info.X_(iDistLat) -
                 (slaver_trace->wid * 0.5));
    }
    ...
    if (fabs(master_trace->kf_info.X_(iDistLat)) < (master_trace->wid * 0.5)) {
      ...
      distance2 =
          (master_trace->kf_info.X_(iDistLat) > 0.0)
              ? (master_trace->kf_info.X_(iDistLat) - (master_trace->wid * 0.5))
              : (-1.0 * master_trace->kf_info.X_(iDistLat) -
                 (master_trace->wid * 0.5));
    }

    if ((distance1 == 0.0) && (distance2 == 0.0)) {
      if (slaver_trace->kf_info.X_(iDistLong) >
          master_trace->kf_info.X_(iDistLong)) {
        return false;
      }
    } else {
      // 本车道【-2，2】
      if ((distance1 < 2.0) && (distance2 < 2.0) &&
          (fabs(slaver_trace->kf_info.X_(iDistLat) -
                master_trace->kf_info.X_(iDistLat)) <
           ((slaver_trace->wid + master_trace->wid) * 0.5 * 0.5))) {
        if ((slaver_trace->kf_info.X_(iDistLong) >
             master_trace->kf_info.X_(iDistLong)) &&
            (slaver_trace->status < master_trace->status)) {
          return false;
        }
      }
    }
 ...
 * @return {*}
 */
bool compare_static_trace_with_dyntrace(gridTrack_t *static_trace,
                                        trackTable_strcut *dynamic_trace) {
  bool result = true;
  rect_point_struct r1, r2;
  creat_rect_box_point(static_trace->kf_info.X_(0), static_trace->kf_info.X_(1),
                       static_trace->len, static_trace->wid, 0.0, r1);

  if ((fabs(static_trace->kf_info.X_(0) -
            dynamic_trace->KalmanInfo.StateEst(0)) > 10.0 &&
       fabs(static_trace->kf_info.X_(1) -
            dynamic_trace->KalmanInfo.StateEst(1)) > 10.0)) {
  } else {
    creat_rect_box_point(dynamic_trace->KalmanInfo.StateEst(0),
                         dynamic_trace->KalmanInfo.StateEst(1),
                         dynamic_trace->ExtendInfo.Length,
                         dynamic_trace->ExtendInfo.Width,
 ...
 * @return {*}
 */
angle_enum compute_two_angle_position(std::pair<double, double> angle1,
                                      std::pair<double, double> angle2) {
  double opening_angle1 = angle1.second - angle1.first;
  double opening_angle2 = angle2.second - angle2.first;

  double Opening_angle = 0;

  std::vector<double> angle_list{angle1.first, angle1.second, angle2.first,
                                 angle2.second};
  ...
  if (Opening_angle >= (opening_angle1 + opening_angle2))  // 不相交
  {
    ...
    if ((Opening_angle == opening_angle1) ||
        (Opening_angle == opening_angle2))  // 某角度完全重叠
    {
 ...
 * @return {*}
 */
double compute_two_angle_iou(std::pair<double, double> angle1,
                             std::pair<double, double> angle2) {
  double opening_angle1 = angle1.second - angle1.first;
  double opening_angle2 = angle2.second - angle2.first;
