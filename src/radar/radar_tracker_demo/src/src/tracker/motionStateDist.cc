                     ...
                     staticPointSet_struct & staticPointSetOut)
{
    ...
    if((fabs(fabs(v_heading) - fabs(vehicleInfo[0].heading)) > (10.0 * DEG2RAD)) &&
        (fabs(fabs(fabs(v_heading) + fabs(vehicleInfo[0].heading) - M_PI)) < (10.0 * DEG2RAD)))
    {
        VehicleReverse = true;
    }
...
//            rectangle_global = vehicleCoor_to_Global(rectangle_vcoor, odometry_xy, odometry_yaw);
            odometryIndex++;

            GetInstallInfo(sensorID_temp, &radar_install_x, &radar_install_y, &radar_install_yaw);

            angle_temp = atan(fabs(radar_install_x/radar_install_y));
            r_temp = sqrtf(radar_install_x*radar_install_x + radar_install_y*radar_install_y);
            radar_vlinear = fabs(r_temp*odometry_yawrate);//由转弯引起的雷达线速度大小
        }
        ...
        if (fabs(odometry_v) < 0.01)
        {
            ...
            if (odometry_yawrate < 0.0)
            {
                radar_theory_v = sqrtf(radar_vlinear*radar_vlinear + odometry_v*odometry_v - 2*odometry_v*radar_vlinear*cos(RADAR_PI - angle_temp));
                //雷达速度（radar_theory_v）方向
                radar_v_yaw = asin(radar_vlinear*sin(angle_temp)/radar_theory_v);
                //目标理论补偿的多普勒速度
                point_theory_v = cos((radar_install_yaw + radar_v_yaw + indata.RadarMeasure[i].Azimuth))*radar_theory_v;
        ...
        */
        ...
        if((fabs(v_diff) < BASE_GATE) ||
            ((fabs(odometry_yawrate) < (DEG2RAD*7.0F)) &&
             (fabs(indata.RadarMeasure[i].VrelLong + radar_theory_v) < BASE_GATE)))
        {
            staticFlag = true;
        }
        ...
        if(staticFlag == false)
        {
            ...
            if(fabs(odometry_yawrate) > (5.0*DEG2RAD))
            {
                if(!VehicleReverse)
                {
                    if(fabs(indata.RadarMeasure[i].VrelLong + odometry_v) < (fabs(odometry_yawrate) * 5.0 + 0.5)) //(0.1 + 0.25*2.0F))
                    {
                        staticFlag = true;
                    }
                }
...
bool isInRectangle(MatrixXd rectangle_global, staticPoint_struct pointIn)
{
    ...
    if (((dot_multi_A >=0)&&(dot_multi_B >=0)&&(dot_multi_C >=0)&&(dot_multi_D >=0))||
            ((dot_multi_A <=0)&&(dot_multi_B <=0)&&(dot_multi_C <=0)&&(dot_multi_D <=0)))
    {
        return true;
    }
    else{
 ...
 * @return {*}
 */
MatrixXd vehicleCoor_to_Global(MatrixXd rectangle_vcoor, VectorXd v_global, float v_yaw)
{
    MatrixXd rectangle_global = MatrixXd(4, 2);
    MatrixXd matrix_T = MatrixXd(2, 2);
    MatrixXd rectangle_global_temp = MatrixXd(4, 2);
    matrix_T << cos(v_yaw), sin(v_yaw),
            -sin(v_yaw), cos(v_yaw);
...
bool myransac(MatrixXd dataIn, int16_t datalen, int16_t n_shuffle, int16_t itersNum, float dis, float per, VectorXd &vEst, VectorXi &inlierIndex, uint16_t *inlier_num)
{
    ...
    Eigen::MatrixXd randvalue = (Eigen::MatrixXd::Random(1, itersNum * n_temp)).array() * data_num + data_num;
    ...
    for (int16_t i = 0; i < itersNum; i++)
    {
        uint16_t xx = ((v_estim2.col(i) - dataIn.col(2)).array().abs() < dis).count();
