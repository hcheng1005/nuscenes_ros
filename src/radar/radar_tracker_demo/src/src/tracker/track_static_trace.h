...
double kf_update(Eigen::VectorXd &X, Eigen::MatrixXd &P, Eigen::VectorXd preZ,
                 VectorXd measZ, double weight, gridTrack_t &trace);

bool trace_assign_with_point(gridTrack_t &trace,
...
double compute_min_angle(const double x_pos, const double y_pos,
                         const double len, const double wid);
void trace_abnormal_trace_delete2(gridTrack_t *trace_list,
                                  vehicleInfo_struct &vehicleInfo);
...
void static_trace_assign_with_box(
    gridTrack_t *static_track_list, std::vector<det_box_t> &det_box_list,
    std::vector<std::vector<uint16_t>> &dyn_trace_assign_list);
