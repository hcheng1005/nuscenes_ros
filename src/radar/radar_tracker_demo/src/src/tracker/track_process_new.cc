                     ...
                     std::vector<orin_trace_info_t> &trace_info_list_all)
{
    ...
    for (const auto sub_trace : valid_trace_list)
    {
        ...
        new_trace_info.track_type = (static_track_list[sub_trace].status == ACTIVATE) ? (ACTIVE_STATIC_TRACK) : (UNACTIVE_STATIC_TRACK);
    ...
    for (const auto sub_trace : dynamic_trace_list)
    {
        ...
        new_trace_info.track_type = (dyn_track_list[sub_trace].trackState == TRACK_STATE_ACTIVE) ? (ACTIVE_DYNAMIC_TRACK) : (UNACTIVE_DYNAMIC_TRACK);
            ...
            if (sub_pair.iou > 0.0) // 存在交叉
            {
                    ...
                    (trace_info_list.at(trace_idx).track_type == UNACTIVE_STATIC_TRACK))
                {
                    ...
                    if (sub_det_box.box_feature == STATIC_BOX)
                    {
                        ...
                        {
                            box_ratio = (sub_pair.trace_box_s > sub_pair.det_box_s) ? (sub_pair.trace_box_s / sub_pair.det_box_s) : (sub_pair.det_box_s / sub_pair.trace_box_s);
                            ...
                            else
                            {
                                ...
                                {
                                    distance_2_zero = (static_trace.kf_info.X_(iDistLat) > 0.0) ? (static_trace.kf_info.X_(iDistLat) - 0.5 * static_trace.wid) : (-static_trace.kf_info.X_(iDistLat) + 0.5 * static_trace.wid);
                                }
                                else
                                {
                                    distance_2_zero = 0.0;
        ...
        if (((sub_det_box.valid & (1 << VALID)) == (1 << VALID)) || (sub_det_box.assigned_obj == UNACTIVE_DYNAMIC_TRACK))
        {
            ...
            */
            ...
            else if ((sub_det_box.box_feature == DYNAMIC_BOX) && (sub_det_box.member_num > 1))
            {
                ...
                {
                    double iou_sp = (sub_pair.trace_box_s < sub_pair.det_box_s) ? (sub_pair.box_overlap / sub_pair.trace_box_s) : (sub_pair.box_overlap / sub_pair.det_box_s);
    ...
    for (auto slaver_trace_ang : trace_angle_list)
    {
        ...
        for (auto master_trace_ang : trace_angle_list)
        {
            ...
            if ((slaver_static_trace != nullptr) && (master_dynamic_trace != nullptr))
            {
                ...
                {
                    dis_to_zero_1 = (master_dynamic_trace->KalmanInfo.StateEst(iDistLat) > 0.0) ? (master_dynamic_trace->KalmanInfo.StateEst(iDistLat) - (master_dynamic_trace->ExtendInfo.Width * 0.5)) : (-1.0 * master_dynamic_trace->KalmanInfo.StateEst(iDistLat) - (master_dynamic_trace->ExtendInfo.Width * 0.5));
                }
                ...
                {
                    dis_to_zero_2 = (slaver_static_trace->kf_info.X_(iDistLat) > 0.0) ? (slaver_static_trace->kf_info.X_(iDistLat) - (slaver_static_trace->wid * 0.5)) : (-1.0 * slaver_static_trace->kf_info.X_(iDistLat) - (slaver_static_trace->wid * 0.5));
                }
...
bool compare_static_trace(gridTrack_t *slaver_trace, gridTrack_t *master_trace)
{
    ...
    else
    {
        ...
        {
            distance1 = (slaver_trace->kf_info.X_(iDistLat) > 0.0) ? (slaver_trace->kf_info.X_(iDistLat) - (slaver_trace->wid * 0.5)) : (-1.0 * slaver_trace->kf_info.X_(iDistLat) - (slaver_trace->wid * 0.5));
        }
        ...
        {
            distance2 = (master_trace->kf_info.X_(iDistLat) > 0.0) ? (master_trace->kf_info.X_(iDistLat) - (master_trace->wid * 0.5)) : (-1.0 * master_trace->kf_info.X_(iDistLat) - (master_trace->wid * 0.5));
        }
