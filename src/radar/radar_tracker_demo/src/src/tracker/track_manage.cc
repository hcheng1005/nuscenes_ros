...
void track_associate_PointThr(trackTable_strcut *trackInfo)
{
  ...
  for (uint16_t track_i = 0; track_i < MAXTRACKS; track_i++)
  {
    ...
    range_ = sqrt(pow(trace->KalmanInfo.StateEst(iDistLat), 2.0) + pow(trace->KalmanInfo.StateEst(iDistLong), 2.0));
    azi_ = ComputeAngle(trace->KalmanInfo.StateEst(iDistLat), trace->KalmanInfo.StateEst(iDistLong));
    vel_ = (trace->KalmanInfo.StateEst(iDistLat) * trace->KalmanInfo.StateEst(iVrelLat) +
...
void track_manage_confirmedtrace(trackTable_strcut *trace)
{
  ...
  {
      if((trace->ExtendInfo.ProbOfExist < (TRACE_PROB_LIMIT * TRACE_DELETE_PERCENT)) || \
  ...
  {
      if((trace->ExtendInfo.ProbOfExist < (TRACE_PROB_LIMIT * (TRACE_DELETE_PERCENT - 0.05))) || \
                                   ...
                                   vehicleInfo_struct *vehicleInfo)
{
  // NM滑窗逻辑
  trace->TCount++;
  trace->ManageInfo.MN_Logic_1 = (trace->ManageInfo.MN_Logic_1 << 1U) & 0xFFFFFFFF;
  trace->ManageInfo.MN_Logic_2 = (trace->ManageInfo.MN_Logic_2 << 1U) & 0xFFFFFFFF;
  ...
  if ((trace->ExtendInfo.ProbOfExist < 0.3F) ||
      ((trace->ManageInfo.detect2missCount >= 2) && (trace->TCount <= 5))) // 提前删除逻辑
  {
    track_clear(trace);
    return;
  }
  ...
  if(((trace_size < 2.0) && (trace->ExtendInfo.ProbOfExist > (TRACE_PROB_LIMIT * TRACE_ACTIVE_SAMLL_TAR)) ) ||  \
        (trace->ExtendInfo.ProbOfExist > (TRACE_PROB_LIMIT * TRACE_ACTIVE_PERCENT*scale)))
  {
  ...
  // 计算MN数值
  uint8_t nm[2] = {0U, 0U};
  uint16_t scanNum = (trace->TCount > trace->ManageInfo.NM_SUM) ? trace->ManageInfo.NM_SUM : trace->TCount;
  ...
  if(((trace_size < 0.64) && (trace->ExtendInfo.ProbOfExist > (TRACE_PROB_LIMIT * TRACE_FINAL_CONFIRM_PERCENT_EX_SMALL))) ||\
      (trace->ExtendInfo.ProbOfExist > (TRACE_PROB_LIMIT * TRACE_FINAL_CONFIRM_PERCENT_BIG*scale)))
  {
...
//          }
  ...
  // 临时航迹删除逻辑
  if ((trace->TCount > 10) && (trace->ExtendInfo.ProbOfExist < (TRACE_PROB_LIMIT * 0.5)))
  {
    track_clear(trace);
  }
  ...
  // 删除特定区域未起始的航迹
  if((fabs(atan2(trace->KalmanInfo.MeasPre(0), trace->KalmanInfo.MeasPre(1) - 3.0)) > (M_PI_4)))
  {
    //
                           ...
                           trackTable_strcut *trace)
{
  ...
  for (uint16_t idx = 0; idx < MAXTRACKS; idx++)
  {
    ...
    if ((fabs(confirmTrkInfo->ExtendInfo.box_theta) >
         (80.0 / 180.0 * 3.14)) &&
        (fabs(confirmTrkInfo->ExtendInfo.box_theta) <
         (100.0 / 180.0 * 3.14)) &&
 ...
 * @return {*}
 */
void track_birth(std::vector<RadarMeasure_struct> &global_point, trackTable_strcut *trackInfo, vehicleInfo_struct *vehicleInfo)
{
  uint16_t trackId = 0;
                          ...
                          fitInfo_struct *fitInfo)
{
  ...
  fitInfo->validNum = ValidNum;
   ...
   auto lat_maxmin_val = std::minmax_element(dis_lat_list.begin(), dis_lat_list.end());
   auto long_maxmin_val = std::minmax_element(dis_long_list.begin(), dis_long_list.end());

   mCenter <<  (0.5*(*lat_maxmin_val.second + *lat_maxmin_val.first)), \
                 0.5 * (*long_maxmin_val.second + *long_maxmin_val.first),
                 sum_v;

    fitInfo->mCenter = mCenter;

    if(((fitInfo->clutter_num / clusterPtr.size()) > (0.8)) && (fabs(mCenter(0)) > 20.0))
    {
      return false;
    }
...
void track_InitTrack(trackTable_strcut *trackInfo, uint16_t valid_trace_idx,
                     fitInfo_struct *fitInfo, std::vector<RadarMeasure_struct> &global_point)
{
...
void Track_Init_Corners(trackTable_strcut *trace, bool extendFlag)
{
  ...
  else
  {
    ComputeCornerPos(trace->KalmanInfo.StateEst(iDistLat),
                     trace->KalmanInfo.StateEst(iDistLong),
                     trace->ExtendInfo.box_theta, half_Width,
                     half_length, &trace->ExtendInfo.CornerPos[0][0]);
  }
}
...
void track_merge2(trackTable_strcut *trackInfo)
{
  ...
  for (uint16_t idx1 = 0; idx1 < MAXTRACKS; idx1++)
  {
    ...
    trace_s1 = trackInfo[idx1].ExtendInfo.Length * trackInfo[idx1].ExtendInfo.Width;
    ...
    for (uint16_t idx2 = 0; idx2 < MAXTRACKS; idx2++)
    {
      ...
      trace_s2 = trackInfo[idx2].ExtendInfo.Length * trackInfo[idx2].ExtendInfo.Width;
      ...
      {
        if (abs((int)trackInfo[idx1].TCount - (int)trackInfo[idx2].TCount) < 5.0)
        {
      ...
      creat_rect_box_point(trace_old->KalmanInfo.StateEst(0), trace_old->KalmanInfo.StateEst(1),
                           trace_old->ExtendInfo.Length, trace_old->ExtendInfo.Width,
                           -1.0 * trace_old->ExtendInfo.box_theta, r1);

      creat_rect_box_point(trace_young->KalmanInfo.StateEst(0), trace_young->KalmanInfo.StateEst(1),
                           trace_young->ExtendInfo.Length, trace_young->ExtendInfo.Width,
      ...
      {
          //存在相交的情况下，进一步比较速度差
          if( ((fabs(trace_old->KalmanInfo.StateEst(iVrelLat) - trace_young->KalmanInfo.StateEst(iVrelLat)) < 2.0) &&
              (fabs(trace_old->KalmanInfo.StateEst(iVrelLong) - trace_young->KalmanInfo.StateEst(iVrelLong)) < 1.0 )) ||
              (iou_val > 0.3))
          {
 ...
 * @return {*}
 */
void trace_merge_newtrace(trackTable_strcut *trace_old, trackTable_strcut *trace_young)
{
  ...
  sumWeight = trace_old->ExtendInfo.ProbOfExist + trace_young->ExtendInfo.ProbOfExist;
  w1 = (double)(trace_old->ExtendInfo.ProbOfExist / sumWeight);
  w2 = (double)(trace_young->ExtendInfo.ProbOfExist / sumWeight);

  // TODO: 状态融合策略需要修改
  X_new = trace_old->KalmanInfo.StateEst * w1 + trace_young->KalmanInfo.StateEst * w2;
