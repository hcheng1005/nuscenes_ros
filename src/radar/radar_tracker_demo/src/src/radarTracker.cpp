 ...
 * @return {*}
 */
std::vector<radar_cluster_t> point_cluster(
    std::vector<radar_point_t> &new_meas) {
...
void trace_birth(std::vector<radar_point_t> &new_meas) {
  ...
  for (auto &sub_cluster : clusters) {
    ...
    std::cout << "new trace:[" << new_x(0) << ", " << new_x(1) << "]"
              << std::endl;

    RandomMatrice::Tracker trace(new_x, sub_cluster.len, sub_cluster.wid,
                                 global_id);
...
int radar_track_main(std::vector<radar_point_t> &new_meas) {
  ...
  if (VISUALIZATION) {
    ...
    for (auto &trace : TraceList) {
      X_ = trace.get_X();
      auto point =
          cv::Point2f((X_(0) + 100) / 200 * 800, 600 - (X_(1) / 100 * 600));
