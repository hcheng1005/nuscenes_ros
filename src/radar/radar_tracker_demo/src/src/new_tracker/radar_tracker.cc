...
RadarTracker::RadarTracker(uint new_id, float center_lat, float center_long,
  ...
  trace_status.trace_kalman.X(iVrelLat) =
      vr * sin(atan2(center_lat, center_long));
  trace_status.trace_kalman.X(iVrelLong) =
      vr * cos(atan2(center_lat, center_long));
...
void RadarTracker::trace_predict() {
  trace_status.trace_kalman.X =
      trace_status.trace_kalman.F * trace_status.trace_kalman.X;
  trace_status.trace_kalman.P = trace_status.trace_kalman.F *
                                    trace_status.trace_kalman.P *
                                    trace_status.trace_kalman.P.transpose() +
                                trace_status.trace_kalman.Q;

  computeJacMat();
}

void RadarTracker::update_kinematic(const VectorXf& Z) {
  MatrixXf S = trace_status.trace_kalman.H * trace_status.trace_kalman.P *
                   trace_status.trace_kalman.H.transpose() +
               trace_status.trace_kalman.R;
  MatrixXf K = trace_status.trace_kalman.P *
               trace_status.trace_kalman.H.transpose() * S.inverse();

  VectorXf preX(3);
  double vr_ = (trace_status.trace_kalman.X(iDistLat) *
                    trace_status.trace_kalman.X(iVrelLat) +
                trace_status.trace_kalman.X(iDistLong) *
                    trace_status.trace_kalman.X(iVrelLong)) /
               (sqrt(pow(trace_status.trace_kalman.X(iDistLat), 2.0) +
                     pow(trace_status.trace_kalman.X(iDistLong), 2.0)));

  preX << trace_status.trace_kalman.X(iDistLat),
      trace_status.trace_kalman.X(iDistLong), vr_;

  // std::cout << "Z: " << Z.transpose() << std::endl;
  // std::cout << "X: " << preX.transpose() << std::endl;
  // std::cout << "X_old: " <<  trace_status.trace_kalman.X.transpose() <<
  // std::endl;

  trace_status.trace_kalman.X = trace_status.trace_kalman.X + K * (Z - preX);
  trace_status.trace_kalman.P =
      trace_status.trace_kalman.P - K * S * K.transpose();

  // std::cout << "K * (Z - preX): " << (K * (Z - preX)).transpose() <<
  // std::endl; std::cout << "X_new: " <<
  // trace_status.trace_kalman.X.transpose() << std::endl;
}

void RadarTracker::update_physical(float new_len, float new_wid,
                                   float new_theta) {
  trace_status.trace_shape.len +=
      (new_len - trace_status.trace_shape.len) * 0.9;
  trace_status.trace_shape.wid +=
      (new_wid - trace_status.trace_shape.wid) * 0.9;
}
